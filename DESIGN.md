...
- **Body:** the platform system sans-serif stack for legibility and zero font-download cost.
- **Utility:** system monospace is reserved for small product data and category labels.
...
On fine-pointer devices, the product stage rests in Foil grey (`#D7DAE0`) and reveals the selected formula color on hover or formula-control keyboard focus. Touch layouts keep the selected formula color visible because hover is unavailable. The color transition is restrained and never shifts layout.
...
The responsive target is 320–1366px across portrait and landscape layouts, including safe areas on iOS/iPadOS 16+ and current-to-two-versions-back Chrome on Android. The page remains framework-free, uses optimized local campaign assets, and skips pointer-only effects on touch hardware.
...
Both pages use the same sticky top bar at every breakpoint. The SURGE wordmark sits left and `Home` plus `Early access` sit right. A single shared underline position handles every navigation state: the current route uses SURGE Blue at rest, while hover, keyboard focus, and press use Carbon without a color fill or second line. Header height is owned only by the shared stylesheet—64px on phones and 72px from tablet upward—so the pages cannot drift. Contact is removed from primary navigation and appears only in the shared bottom footer. The footer is a single row from tablet upward and stacks on phones.
